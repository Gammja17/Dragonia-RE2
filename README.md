...
| WASD | 카메라 기준 이동 |
| Shift | 탁 = 무적 대시 · 꾹 = 달리기 |
| Space | 뛰기 — 공중에서 연타하면 날갯짓으로 올라간다, 떼면 활공 (기력 없음) |
| C | 날개 접고 떨어지기 |
| 왼클릭 / F (꾹) | 숨결 탄막 — 걸으면서, 날면서 쏜다. 불은 세 갈래, 얼음은 센 한 발, 번개는 연사 (수치는 elements.json 그대로) |
| 오른클릭 | 땅: 물기 · 공중: 내려찍기 |
| Q | 락온 걸기/풀기 |

보스도 난다. 패턴 두어 개마다 땅과 하늘을 오가고, 하늘에서는 플레이어 둘레를 돌며 겨눠 쏜다
(RING → 원뿔 탄, SPIRAL·TWIN_BEAM → 기총소사, CHARGE → 급강하). 물기는 안 닿으니 같이 날아올라 쏘거나 내려찍는다.
